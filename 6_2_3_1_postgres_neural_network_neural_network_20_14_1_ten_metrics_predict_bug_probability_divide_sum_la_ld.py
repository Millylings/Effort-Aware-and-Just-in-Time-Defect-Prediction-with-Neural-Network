...
loss_of_train_history_global = [ ]
accuracy_of_train_history_global = [ ]

#val_loss_of_train_history_global  = [ ]
#val_acc_of_train_history_global  = [ ]

metrics_of_train_evaluate_global = [ ]
metrics_of_test_evaluate_global = [ ]

model_name_count_number_global = 1
...
#    model.add(Activation('relu'))
  ...
  #第五个隐层有3个神经元，激活函数为ReLu，Dropout比例为0.2
    ...
    *******第三步：编译模型*******
    ...
    model.compile(optimizer='adam',loss='mean_squared_error', metrics=['mean_squared_error','mean_absolute_error','mean_absolute_percentage_error','cosine_proximity','acc'])
    #model.compile(optimizer='sgd',loss='mse',metrics=['accuracy'])


    hist = model.fit( remove_nd_la_ld_rexp_X_train, ultimate_y_train, batch_size = 30, nb_epoch = 150, verbose=1) 
...
#    loss_and_metrics = model.evaluate(ultimate_X_train, ultimate_y_train, batch_size=128)
    ...
    """2018-12-17"新添加的Traing result"""
    train_loss = hist.history['loss']
    #val_loss = hist.history['val_loss']
    train_acc = hist.history['acc']
    #val_acc = hist.history['val_acc']
    train_mean_squared_error = hist.history['mean_squared_error']
    train_mean_absolute_error = hist.history['mean_absolute_error']
    train_mean_absolute_percentage_error = hist.history['mean_absolute_percentage_error']
    train_cosine_proximity = hist.history['cosine_proximity']
    
    
    
    training_scores = model.evaluate(remove_nd_la_ld_rexp_X_train, ultimate_y_train )
    training_loss = training_scores[0]
    training_accuracy = training_scores[1]




    
    testing_scores = model.evaluate(remove_nd_la_ld_rexp_X_test, ultimate_y_bug_label_test )
    testing_loss = testing_scores[0]
    testing_accuracy = testing_scores[1]



    model_name_temp = "postgress_model"

    model_name_final = model_name_temp + str(model_name_count_number_global)
    model_name_postfix = model_name_final + ".h5"
    print (model_name_postfix)
    model.save(model_name_postfix)
    model_name_count_number_global = model_name_count_number_global + 1
...
#    print('\nTesting ------------')
    ...
    metrics_of_train_evaluate_global.append(training_scores)
...
training_model_metrics_evaluate_results_final = []
tem_training_result_output = np.array([], dtype=float).reshape(-1,6)
#定义一个空的矩阵
#数据集中有6列所以此处是6,用一个变量random_dataset_width来表示，免得数据集一旦多一列就会出错。
#random_dataset_width = random_dataset.shape[1]用此种方法获得，即可动态变化
#训练集
for i in range(len(metrics_of_train_evaluate_global)):   
    #tem_my_neural_row = my_neural_network_exp_result_global[i]
    #取得list中第一列的数值
    tem_training_result = np.array(metrics_of_train_evaluate_global[i])
    tem_training_result_output = np.vstack((tem_training_result_output,tem_training_result))
    #将第list中第一个数值在垂直方向上和别的数值进行合并
    print(tem_training_result_output)
training_model_metrics_evaluate_results_final.append(tem_training_result_output)
print(training_model_metrics_evaluate_results_final)
...
"""Training Result  2018年12月18日"""
model_train_results = training_model_metrics_evaluate_results_final

store_path_train_result = 'M:/paper/experiment_result_defect_probability_divide_sum_la_ld/recall_ten_metrics_nn_predect_bug_probolity_diveded_sum_la_ld_one/model_training_results/' 
project_name_training_result = 'postgres_training_result_train_dataset' 
store_exp_result(store_path_train_result, project_name_training_result, model_train_results)
